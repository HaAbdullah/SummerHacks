"""Load data/parts.json into the parts table.
...
data/parts.json is the editable source; the table is what the API reads.
"""
...
def main() -> None:
    ...
    catalogue = {car: slots for car, slots in catalogue.items() if not car.startswith("_")}

    written = 0
    for car_id, slots in catalogue.items():
        # Clear this car's rows first, so a part removed from the file actually
        # disappears rather than lingering from a previous run.
    ...
    print(f"\nWrote {written} parts via the {'supabase' if _is_supabase() else 'json'} store.")
